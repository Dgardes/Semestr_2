...
private:
	...
	//
	// додатково до лекції №24
	//

	//Counting Sort 
	public: void countingSortByGrade(std::vector<Student>& arr)
	{
		if (arr.empty()) return;

		int maxGrade = 100;
		std::vector<int> count(maxGrade + 1, 0);
		std::vector<Student> output(arr.size());

		//рахуємо кількість кожної оцінки
		for (size_t i = 0; i < arr.size(); i++) {
			count[arr[i].grade]++;
		}

		//модифікація count, щоб він містив фактичні позиції елементів
		...
		for (int i = (int)arr.size() - 1; i >= 0; i--) {
			output[count[arr[i].grade] - 1] = arr[i];
			count[arr[i].grade]--;
			swaps++; //фіксуємо переміщення елемента для статистики
		}
	...
	//Radix Sort для ID
	public: void radixSortById(std::vector<Student>& arr)
	{
		if (arr.empty()) return;

		//знаходимо максимальний ID, щоб знати скільки розрядів обробляти
		int maxId = arr[0].id;
		...
		//виклик Counting Sort для кожного розряду 
		for (int exp = 1; maxId / exp > 0; exp *= 10) {
			countingSortForRadix(arr, exp);
		}
	}
	
	//допоміжна функція для Radix Sort
	public: void countingSortForRadix(std::vector<Student>& arr, int exp)
	{
		std::vector<Student> output(arr.size());
		int count[10] = { 0 };

		//підрахунок входження цифр у поточному розряді
		for (size_t i = 0; i < arr.size(); i++) {
			int digit = (arr[i].id / exp) % 10;
			count[digit]++;
		}

		//зміна count на префіксні суми
		for (int i = 1; i < 10; i++) {
			count[i] += count[i - 1];
		}
	...
	//Merge Sort для імен
	public: void mergeSortByName(std::vector<Student>& arr)
	{
		if (arr.empty()) return;
		mergeSortByNameRecursive(arr, 0, (int)arr.size() - 1);
	}
	
	//допоміжні функції для Merge Sort за ім'ям
	public: void mergeSortByNameRecursive(std::vector<Student>& arr, int left, int right)
	{
		if (left < right) {
			int mid = left + (right - left) / 2;
			mergeSortByNameRecursive(arr, left, mid);
			mergeSortByNameRecursive(arr, mid + 1, right);
			mergeByName(arr, left, mid, right);
		}
	}

	public: void mergeByName(std::vector<Student>& arr, int left, int mid, int right)
	{
		int n1 = mid - left + 1;
		int n2 = right - mid;
	...
	//інтерполяційний пошук за ID
	public: int interpolationSearchById(const std::vector<Student>& arr, int targetId)
	{
		int low = 0, high = (int)arr.size() - 1;

		while (low <= high && targetId >= arr[low].id && targetId <= arr[high].id) {
			if (low == high) {
				if (arr[low].id == targetId) return low;
				return -1;
			}

			//формула інтерполяції
			int pos = low + (((double)(high - low) / (arr[high].id - arr[low].id)) * (targetId - arr[low].id));

			if (arr[pos].id == targetId) return pos;
			if (arr[pos].id < targetId) low = pos + 1;
			else high = pos - 1;
		}
		return -1;
	}

	//бінарний пошук з діапазоном за оцінкою
	public: std::pair<int, int> binarySearchRangeByGrade(const std::vector<Student>& arr, int targetGrade)
	{
		int first = -1, last = -1;
		int low = 0, high = (int)arr.size() - 1;
		...
		while (low <= high) {
			int mid = low + (high - low) / 2;
			if (arr[mid].grade == targetGrade) {
				first = mid;
				high = mid - 1; //продовжуємо шукати вліво
			}
			else if (arr[mid].grade < targetGrade) {
				low = mid + 1;
			}
			else {
				high = mid - 1;
		...
		while (low <= high) {
			...
			if (arr[mid].grade == targetGrade) {
				last = mid;
				low = mid + 1; //продовжуємо шукати вправо
			}
			else if (arr[mid].grade < targetGrade) {
				low = mid + 1;
			}
	...
	//бінарний пошук за ім'ям
	public: int binarySearchByName(const std::vector<Student>& arr, const std::string& targetName)
	{
		int low = 0, high = (int)arr.size() - 1;
		while (low <= high) {
			int mid = low + (high - low) / 2;
			if (arr[mid].name == targetName) return mid;

			if (arr[mid].name < targetName) low = mid + 1;
			else high = mid - 1;
		}
		return -1;
	}
	...
	public: int getComparisons() const { return comparisons; }
	public: int getSwaps() const { return swaps; }
